...
def test_is_in_china():
    chnroutes = ["1.1.8.0/24", "223.20.0.0/15",
                 "210.28.0.0/14", "210.15.32.0/19", "163.0.0.0/16"]
    cnet = ChinaNet(chnroutes, [], False)
    cnet_rfc1918 = ChinaNet(chnroutes, [], True)
    for ip in ["1.1.8.0", "1.1.8.255", "223.20.0.23", "223.20.0.1",
               "223.21.255.255", "210.28.0.0", "210.15.32.25", "163.0.2.3"]:
        assert cnet.is_in_china(ip)
    for ip in ["1.1.7.255", "1.1.9.0", "23.0.20.223", "163.2.0.3",
               "254.3.4.5", "8.8.8.8", "257.1223.3.3"]:
        assert not cnet.is_in_china(ip)
    assert cnet_rfc1918.is_in_china("192.168.2.3")
    assert cnet_rfc1918.is_in_china("10.2.3.4")
    assert cnet_rfc1918.is_in_china("172.23.2.3")
